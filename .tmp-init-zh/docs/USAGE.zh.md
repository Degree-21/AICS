...
- 以 docs/templates/* 的模板与规格为唯一事实来源
...
请基于上述规格生成实现代码。
约束：
- 严格遵循输入/输出与错误码；保持纯函数或契约一致
- 使用本项目约定的技术栈与目录结构
输出：
- 仅提供要修改/新增的文件路径与完整内容；不生成无关文件
- 简述设计权衡（≤5 句）
```
...
```text
请为上述实现生成测试，覆盖正向/负向/边界，并包含：
- 契约/Schema 校验
- 日志断言（错误路径）
- 资源获取/释放检查（如适用）
输出：测试文件路径与完整内容，并列出测试清单与断言要点。
```

- 失败时的调试与回写（“用户消息”）

```text
以下用例失败，请按系统化调试工作流处理，并更新规格与测试：
失败复现：<<< 最小输入与步骤 >>>
日志片段：<<< 错误日志与栈 >>>
输出：
