...
```text
请先扫描并分析项目根目录（如 package.json, go.mod, pom.xml, requirements.txt 等）以识别技术栈。
将你对项目技术栈、目录结构、代码风格与依赖的理解整理为 Markdown 格式，保存到 `aics-docs/project-context.md` 中。
如果项目为空，请推荐合适的技术栈并生成初始脚手架结构，同时更新该上下文文档。
```
